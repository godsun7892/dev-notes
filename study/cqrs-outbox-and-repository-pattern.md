...
|------|------|
| Date | 2026-04-10 |
| Context | Worker/GM 레이어 도메인 정의를 위해 영속성 패턴을 학습 |
...
| **Active Record** (Django) | 모델 객체가 자기 자신을 저장 (`erp.save()`) | 웹 CRUD, 어드민 |
...
```python
def execute_trade(seller, buyer, offer):
    UPDATE postgres.erp ...        # ① 상태
...
- ② 성공, ① 실패 → 이벤트는 있는데 상태 안 바뀜 → agent가 잘못된 상태 봄
...
def execute_trade(seller, buyer, offer):
    BEGIN postgres transaction
        UPDATE erp SET cash = ... WHERE erp_id = seller
        UPDATE erp SET cash = ... WHERE erp_id = buyer
        INSERT INTO outbox (event_payload, published) VALUES (..., false)
...
LinkedIn/Uber가 Kafka를 쓰는 건 초당 수백만 events 처리 때문이지, outbox 패턴이 Kafka를 요구해서가 아님. Redis + Celery로 outbox 구현해도 정통.
...
- Gary Bernhardt — Functional Core, Imperative Shell (Boundaries talk)
