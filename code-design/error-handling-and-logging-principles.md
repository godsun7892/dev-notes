...
흔한 오해: "API → Service → Repository → DB" 같은 단순 수직 스택.
...
```
                ...
                └──────────┬───────────┘
                           │
                ┌──────────────────────┐
                │  Adapter Layer       │   ← 외부 인터페이스 (Tool, CLI 등)
                └──────────┬───────────┘
                           │
                ┌──────────────────────┐
                │  Orchestration       │
...
| | 도메인 실패 | 인프라 실패 |
|---|------|------|
| **예** | "잔액 부족", "재고 부족" | "DB 연결 끊김", "레코드가 DB에 없음" |
| **본질** | 비즈니스 규칙 안의 정상 시나리오 | 시스템에 문제가 있는 비정상 |
...
```python
# 도메인 실패: 결과 객체로
def transfer_funds(from_account, to_account, amount) -> TransferResult:
    if from_account.balance < amount:
        return TransferResult(success=False, reason="insufficient_balance")
...
| **Orchestration (Service/Worker)** | 데이터 로드 + 엔진 호출 + 영속화 | Repository 예외 catch 가능. 재시도/로깅/위로 전파 결정 |
| **Adapter Layer (Tool/CLI)** | 외부와의 인터페이스 | 예외 → 외부가 이해할 수 있는 구조화 응답으로 변환 |
| **HTTP/API** | 외부 시스템 경계 | 안 잡힌 예외 → HTTP 상태 코드 매핑 (NotFound → 404, 그 외 → 500) |

핵심 원칙: **"의미 있게 처리할 수 있는 가장 가까운 곳에서만 잡는다."**

```python
# 시나리오: Repository NotFoundError → Adapter에서 변환
def cancel_order_handler(order_id):
    try:
        result = cancel_order_flow(order_id, ...)  # NotFoundError 가능
    except NotFoundError:
        # ← 여기서 잡는 이유: 호출자가 다음 행동 결정하려면 정보 필요
        return error("ORDER_NOT_FOUND", "This order no longer exists.")
    except RepositoryError:
        # 진짜 인프라 장애 → 호출자가 어쩔 도리 없음 → 위로 전파
        raise
    return success(...)
```
...
|---|---|---|
| **누가 보나** | Python 코드 (try/except) | 외부 클라이언트 (응답 JSON) |
...
```python
try:
    order = order_repo.get(order_id)  # ← Exception (control flow)
except NotFoundError:
    return error("ORDER_NOT_FOUND", "...")  # ← Error code (외부 contract)
```

### 왜 둘 다 필요한가

**Exception만 쓰면**: 외부에 던질 응답 형식이 표준화 안 됨. 어떤 에러가 가능한지 클라이언트가 못 앎.

**Error 코드만 쓰면**: 매 호출이 `if result.code == "...":` 보일러플레이트. Pythonic하지 않음 (Rust/Go의 Result 패턴).

```python
# Result 패턴 (Python에선 안 좋음)
result = account_repo.get(seller_id)
if not result.success:
    if result.code == "not_found":
        ...
seller = result.account

result = account_repo.get(buyer_id)
if not result.success:
    ...
buyer = result.account

# Exception (Pythonic, EAFP)
seller = account_repo.get(seller_id)
buyer = account_repo.get(buyer_id)
# 에러는 한 번만 catch
```
...
def cancel_order_flow(order_id):
    try:
        ...
        seller = account_repo.get(order.seller_id)
        config = configs[tenant_id]  # 여기도 KeyError 가능
        ...
    except KeyError as e:
        # 어디서 난 KeyError?
        # order 미스? account 미스? config dict 미스?
        # 같은 type이라 구분 불가
        ...
```
...
class InMemoryAccountRepo:
    def get(self, account_id):
        try:
            return self._store[account_id]  # KeyError 발생 가능
        except KeyError:
...
Adapter      ← 외부 호출 입출력 (INFO/DEBUG)
Service/Worker ← ★ 메인 로깅 레이어 (INFO/WARNING/ERROR)
Engine       ← 로그 X (순수 함수)
...
```
❌ Bad: NotFoundError가 Repository → Service → Adapter에서 각각 로그
...
```python
logger.info(f"Order ord_123 placed by user_456 at $99")
```
→ 정규식 파싱 필요. 검색/집계 어려움.
...
```
→ Loki/Elasticsearch에서 `{event="order_placed", user_id="user_456"}` 같은 쿼리 즉시.
...
```
{"event": "request_received", "trace_id": "abc123"}
{"event": "queue_dispatched", "trace_id": "abc123"}
{"event": "worker_processing", "trace_id": "abc123"}
{"event": "db_committed", "trace_id": "abc123"}
```
...
try:
    ...
    raise NotFoundError("Account not found")  # 의미 없음
```
...
class AccountRepository:
    def transfer(self, from_id, to_id, amount):
        if from_account.balance < amount:
            raise InsufficientFundsError  # Repository는 도메인 규칙 모름
```
...
```python
def transfer_funds(seller_id, buyer_id, amount):
    seller = account_repo.get(seller_id)  # 엔진이 Repository 알면 안 됨
```
→ 엔진은 객체를 받고 객체를 뱉음.
...
Service:    logger.error("not found")
Adapter:    logger.error("not found")
```
→ 로그 3배. 대시보드 노이즈.
...
| **Adapter** | 예외 → 외부 응답으로 변환 |
| **Exception vs Error code** | Exception = control flow, Error code = 외부 contract. 둘 다 필요 |
...
- [The Twelve-Factor App](https://12factor.net/) — Logs as event streams (factor XI)
- Robert C. Martin — *Clean Code*, Error Handling chapter
- Michael Nygard — *Release It!*, Stability patterns
- [Python EAFP](https://docs.python.org/3/glossary.html#term-EAFP) — 공식 용어집
- [Python "Errors and Exceptions" 튜토리얼](https://docs.python.org/3/tutorial/errors.html)
- [structlog](https://www.structlog.org/) — Python 구조화 로깅 라이브러리
- [OpenTelemetry — Trace context](https://opentelemetry.io/docs/concepts/signals/traces/)
- Django source — 모델별 `DoesNotExist` 패턴
- SQLAlchemy source — `sqlalchemy.exc` 모듈
